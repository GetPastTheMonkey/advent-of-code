...
while rounds > 0:
    ...
    for (x, y, z, w) in active:
        ...
        min_w = min(min_w, w) if min_w is not None else w
        max_w = max(max_w, w) if max_w is not None else w
                ...
                for w in range(min_w - 1, max_w + 2):
                    ...
                    for x_d in [-1, 0, 1]:
                        for y_d in [-1, 0, 1]:
                            for z_d in [-1, 0, 1]:
                                for w_d in [-1, 0, 1]:
                                    if x_d == 0 and y_d == 0 and z_d == 0 and w_d == 0:
                                        # Don't count myself
                                        continue
                                    if (x + x_d, y + y_d, z + z_d, w + w_d) in active:
                                        active_neighbours += 1

                    # STEP 2: Decide if the cube should be added
                    if (x, y, z, w) in active and 2 <= active_neighbours <= 3:
