...
for candidate in candidates:
    ...
    for i in range(len(candidate)):
        candidate_cookie = Ingredient(
            ...
            candidate_cookie.calories + candidate[i] * ingredients[i].calories
        )

    if candidate_cookie.calories != 500:
        continue

    if any(x <= 0 for x in candidate_cookie):
        continue

    candidate_value = reduce(lambda x, y: x * y, candidate_cookie) / candidate_cookie.calories
